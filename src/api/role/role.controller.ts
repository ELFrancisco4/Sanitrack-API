...
const addRole = async (req: AuthenticatedRequest, res:Response) => { 
    ...
    if(role !== 'Admin') {
        return customResponse.badRequestResponse('You do not have permission to add a role', res);
    }
...
const assignRole = async(req: AuthenticatedRequest, res:Response) =>{
    const role = req.auth.role_id.role_name;
    if(role !== 'Admin') {
        return customResponse.badRequestResponse('You do not have permission to assign a role', res);
    }
